...
try:
    with open('qna.json', 'r') as f:
        qna_set = json.load(f)
    logging.debug(f'Q&A set loaded: {qna_set}')
...
def generate_response(user_input, qna_set):
    try:
        ...
        messages = [{"role": "system", "content": "You are an assistant that answers questions based on a predefined Q&A set."},
                    ...
                    {"role": "system", "content": "You can't said, that your question based on the Q&A set, but you can use the information in the Q&A set to answer the question."}
                    ]
        for item in qna_set:
